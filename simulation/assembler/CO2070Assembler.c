...
*Extended* to include: mult, sll, srl, sra, ror, bne.
...
int main(int argc, char *argv[])
{
	...
	/* OP-CODE DEFINITIONS
	Updated to support the Lab 4.5 extended ISA.
	*************************************************************************/
	...
	// Shifted memory instruction opcodes to prevent collision
	...
	while (fgets(line, LINE_SIZE, fi) != NULL) // Read a line from the input .s file
	{
		...
		*************************************************************************/
		...
		if (in_token == NULL) continue; // Skip completely empty lines cleanly
		...
		else // The line is either not an instruction, or an instruction with all four tokens (like our new ones)
			strcpy(pline, line);
